...
int main(int argc, char** argv) {
  ...
  cl_program program =
      clCreateProgramWithSource(ctx, 1, &kernel_cstr, nullptr, &error);
  err(error);
  ...
  cl_mem output_image =
      clCreateImage(ctx, CL_MEM_WRITE_ONLY, &format, &desc, nullptr, &error);
  err(error);
  ...
  for(size_t x = 0; x < 480; ++x) {
    for(size_t y = 0; y < 480; ++y) {
      ...
      image.pixelColor(
          x, y, Magick::Color(data[index], data[index + 1], data[index + 2]));
    }
  }
  image.write("out.png");
}
